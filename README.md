...
- 云 API 工作区支持阿里云、腾讯云、华为云、火山引擎、百度智能云、萤石云、天地图、七牛云的签名/凭据探测与只读模板
- 对象存储工作区支持阿里云 OSS、腾讯云 COS、百度智能云 BOS、七牛云 Kodo 的桶列表、对象列表、简单上传、流式下载及临时访问凭证
...
3.2 的接口目录按 2026-08-06 可访问的厂商官方文档逐条复核，覆盖微信公众号、微信小程序、企业微信、百度/腾讯/高德地图、飞书、钉钉与代理连通性检查。3.5 起已移除公开规范不足的绿盟 RSAS 条目。
...
原项目共有 87 条接口。3.2 目录共有 138 条：除复核和修订原有 87 条外，累计纳入 51 条真正新增接口，其中 50 条是本次针对 3.1 目录补入的 ID 89–138，另 1 条是此前补入的小程序稳定版 Token（ID 88）；同时新增 9 个功能分组。当前目录为 137 条，已删除绿盟 RSAS 的未核验项。
...
每条正式记录的 `doc_url` 均指向相应官方规范。本地序列化测试项标为 `test-only` 并禁止发送。
